...
class VanillaVAE(nn.Module):
    def __init__(self,
        ...
        for layer_index in self._num_conv_layers:
            layer_number = layer_index + 1
            modules.append(
                nn.Sequential(
                    nn.Conv3d(in_channels, out_channels=conv_filters[layer_index],
                              kernel_size=self.conv_kernels[layer_index], stride=self.conv_strides[layer_index]),
                    nn.BatchNorm2d(conv_filters[layer_index]),
                    nn.LeakyReLU())
            )
            in_channels = conv_filters[layer_index]

        self.encoder = nn.Sequential(*modules)
        self.fc_mu = nn.Linear(hidden_dims[-1]*4, latent_space_dim)
        self.fc_var = nn.Linear(hidden_dims[-1]*4, latent_space_dim)


        # Build Decoder
        modules = []

        self.decoder_input = nn.Linear(latent_space_dim, hidden_dims[-1] * 4)
